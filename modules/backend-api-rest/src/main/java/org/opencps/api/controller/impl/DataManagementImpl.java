...
public class DataManagementImpl implements DataManagement {
	...
	public Response getDictCollection(HttpServletRequest request, HttpHeaders header, Company company, Locale locale,
		...
		try {
			...
			result.getDictCollectionShortModel().addAll(
					DataManagementUtils.mapperDictCollectionShortModelList((List<Document>) jsonData.get(ConstantUtils.DATA)));
	...
	public Response addDictCollection(HttpServletRequest request, HttpHeaders header, Company company, Locale locale,
		...
		try {
			...
			if (oldCollectionTemp == null) {
				...
				dictItemDataTempUtil.updateDictCollectionTemp(user.getUserId(), groupId, collectionCode, collectionCode,
						collectionName, collectionNameEN, description, DataMGTTempConstants.DATA_STATUS_ACTIVE,
						DataMGTTempConstants.DATA_MUST_SYNCHRONIZED, serviceContext);
			}
	...
	public Response getDictgroups(HttpServletRequest request, HttpHeaders header, Company company, Locale locale,
		...
		try {
			...
			result.setTotal(jsonData.getLong(ConstantUtils.TOTAL));
			result.getGroups().addAll(DataManagementUtils.mapperGroupsList((List<Document>) jsonData.get(ConstantUtils.DATA)));
	...
	public Response updateDictgroups(HttpServletRequest request, HttpHeaders header, Company company, Locale locale,
		...
		try {
			...
			if (temp != null) {
				...
				dictItemDataTempUtil.addDictGroupsTemp(user.getUserId(), groupId, code, groupCode, input.getGroupName(),
						input.getGroupNameEN(), input.getGroupDescription(), DataMGTTempConstants.DATA_STATUS_ACTIVE,
						serviceContext);
			}
	...
	public Response getDictgroupsDictItems(HttpServletRequest request, HttpHeaders header, Company company,
		...
		try {
			...
			result.getDictGroupItemModel()
					.addAll(DataManagementUtils.mapperDictGroupItemModelList((List<Document>) jsonData.get(ConstantUtils.DATA)));
...
//				e.printStackTrace();
							...
							if (configObj.has(SyncServerTerm.SERVER_TYPE)
								...
								if (groupId == sc.getGroupId()) {
									pushDictGroupUtil.addPushDictGroup(user.getUserId(), groupId, 
											...
											itemCode, ReadFilePropertiesUtils.get(ConstantUtils.METHOD_ADD_TO_GROUP), serviceContext);
								}
							}
						} catch (Exception e) {
							_log.error(e);
									...
									if (groupId == sc.getGroupId()) {
										if (dictItemGroup != null)
											pushDictGroupUtil.addPushDictGroup(user.getUserId(), groupId, 
													...
													ReadFilePropertiesUtils.get(ConstantUtils.METHOD_REMOVE_FROM_GROUP), serviceContext);
									}
								}
							} catch (Exception e) {
								_log.error(e);
	...
	public Response getDictItems(HttpServletRequest request, HttpHeaders header, Company company, Locale locale,
		...
		try {
			...
			if (ReadFilePropertiesUtils.get(ConstantUtils.VALUE_ADMINISTRATIVE_REGION).equalsIgnoreCase(code))
				groupId = 0;
							...
							if (configObj.has(SyncServerTerm.SERVER_TYPE)
								...
								if (groupId == sc.getGroupId()) {
									pushDictItemUtil.addPushDictItem(user.getUserId(), groupId, 
											...
											SyncServerTerm.METHOD_CREATE, StringPool.BLANK, serviceContext);
								}
							}
						} catch (Exception e) {
							_log.error(e);
						...
						if (configObj.has(SyncServerTerm.SERVER_TYPE)
							...
							if (groupId == sc.getGroupId()) {
								pushDictItemUtil.addPushDictItem(user.getUserId(), groupId, 
										...
										parentItemCode, sibling, SyncServerTerm.METHOD_UPDATE, StringPool.BLANK,
										serviceContext);
							}
						}
					} catch (Exception e) {
	...
	public Response getMetaDataOfDictItemByKey(HttpServletRequest request, HttpHeaders header, Company company,
		...
		if (Validator.isNotNull(dictItem)) {
			...
			String value = ReadFilePropertiesUtils.get(ConstantUtils.ERROR_NOT_PERMISSION);
	...
	public Response getDictCollectionLGSP(HttpServletRequest request, HttpHeaders header, Company company, Locale locale,
		...
		try {
			...
			List<DictCollectionShortModel> dictCollectionList = DataManagementUtils
					.mapperDictCollectionLGSPModelList((List<Document>) jsonData.get(ConstantUtils.DATA));
	...
	public Response getDictItemsLGSP(HttpServletRequest request, HttpHeaders header, Company company, Locale locale,
		...
		try {
			...
			List<DictItemModel> dictItemList = DataManagementUtils
					.mapperDictItemModelListLGSP((List<Document>) jsonData.get(ConstantUtils.DATA), code);
