...
public interface LGSPIntegrationManagement {
	...
	@ApiResponses(value = { @ApiResponse(code = HttpURLConnection.HTTP_OK, message = "Sync id from lgsp system"),
		...
		@ApiResponse(code = HttpURLConnection.HTTP_NOT_FOUND, message = "Not found", response = ExceptionModel.class),
		@ApiResponse(code = HttpURLConnection.HTTP_FORBIDDEN, message = "Access denied", response = ExceptionModel.class) })
	public Response secrectKeyToMD5(@Context HttpServletRequest request, @Context HttpHeaders header,
		@Context Company company, @Context Locale locale, @Context User user,
		@Context ServiceContext serviceContext,@FormParam(value = "secrectKey") String secrectKey);
