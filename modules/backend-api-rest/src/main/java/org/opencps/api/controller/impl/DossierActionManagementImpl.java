...
public class DossierActionManagementImpl implements DossierActionManagement {
	...
	@Override
	public Response getListActions(
		HttpServletRequest request, HttpHeaders header, Company company,
		Locale locale, User user, ServiceContext serviceContext,
		DossierActionSearchModel query, String id) {
		...
		try {
			...
			DossierNextActionResultsModel result =
				new DossierNextActionResultsModel();

			JSONArray jsonData = actions.getNextActionList(
			...
			if (jsonData != null && jsonData.length() > 0) {
				...
				result.getData().addAll(
					DossierActionUtils.mappingToNextActions(jsonData));
				Dossier dossier =
				...
				if (dossier != null) {
					...
					if (Validator.isNotNull(stepCode) && serviceProcessId > 0) {
						...
						if (processStep != null) {
							...
							Date stepDuedate =
								DossierOverDueUtils.getStepOverDue(
									groupId, dossierAction.getActionOverdue(),
									dossierAction.getDueDate());

							result.setStepDueDate(
								stepDuedate != null
									? stepDuedate.getTime() : 0l);
							if (stepDuedate != null) {
...
//								}
							...
							_log.debug(
								stepDuedate +
									"========result.getStepOverdue()=======" +
									result.getStepOverdue());
							// Long releaseDateTimeStamp =
							// (dossier.getReleaseDate() != null ?
							// dossier.getReleaseDate() .getTime(): 0l);
							//
							// Long dueDateTimeStamp = stepDuedate != null ?
							// stepDuedate.getTime() : 0l;
							// if (releaseDateTimeStamp != null &&
							// releaseDateTimeStamp > 0) {
							// if (dueDateTimeStamp != null && dueDateTimeStamp
							// > 0) {
							// long subTimeStamp = releaseDateTimeStamp -
							// dueDateTimeStamp;
							// String strOverDue =
							// DossierUtils.calculatorOverDue(dossier.getDurationCount(),
							// dossier.getDurationUnit(), subTimeStamp,
							...
							// if (dueDateTimeStamp != null && dueDateTimeStamp
							// > 0) {
							// long subTimeStamp = dateNowTimeStamp -
							// dueDateTimeStamp;
							// String strOverDue =
							// DossierUtils.calculatorOverDue(dossier.getDurationCount(),
							// dossier.getDurationUnit(), subTimeStamp,
					...
					if (processStep != null) {
						...
						// ProcessActionTerm.NOT_ASSIGNED) {
						// if (Validator.isNotNull(processStep.getRoleAsStep()))
						// {
						// String[] steps =
						// StringUtil.split(processStep.getRoleAsStep());
						// for (String sc : steps) {
						// if (sc.startsWith("!")) {
						// int index = sc.indexOf("!");
						// String stepCodePunc = sc.substring(index + 1);
						// lstUser.addAll(DossierActionUtils.processRoleAsStepDonedListUser(dossier,
						// stepCodePunc, serviceProcessId, processStep));
						// }
						// else {
						// lstUser.addAll(DossierActionUtils.processRoleAsStepListUser(dossier,
						// sc, serviceProcessId, processStep));
						// }
						// }
						// }
						// else {
						...
						// lstUser.addAll(DossierActionUtils.processRoleListUser(processStepRoleList,
						// serviceProcessId));
						// }
						// }
						...
						// DossierActionUserLocalServiceUtil.getByDossierAndStepCode(dossierId,
						// stepCode);
					...
					if (DossierTerm.DOSSIER_STATUS_NEW.equals(
	...
	@Override
	public Response getActionDetail(
		HttpServletRequest request, HttpHeaders header, Company company,
		Locale locale, User user, ServiceContext serviceContext,
			...
			if (dossier != null) {
				ProcessAction pa =
					ProcessActionLocalServiceUtil.fetchProcessAction(
						GetterUtil.getLong(actionId));

				DossierActionUser dau =
					DossierActionUserLocalServiceUtil.getByD_DID_UID_SC(
						dossierId, dossier.getDossierActionId(),
						user.getUserId(), pa.getPreStepCode());
	...
	@Override
	public Response getListActionsExecuted(
		HttpServletRequest request, HttpHeaders header, Company company,
		Locale locale, User user, ServiceContext serviceContext,
		DossierActionSearchModel query, String id) {
		...
		try {
			...
			if (dossier != null) {
				...
				ServiceProcess serviceProcess =
					ServiceProcessLocalServiceUtil.getByG_PNO(
						groupId, processNo);
				if (serviceProcess != null) {
					...
					List<ProcessSequence> lstSequences =
						ProcessSequenceLocalServiceUtil.findByG_SN(
							groupId, serviceProcess.getProcessNo());
					for (ProcessSequence ps : lstSequences) {
						ProcessSequenceModel psModel =
							new ProcessSequenceModel();
						...
						List<ProcessStep> lstSteps =
							ProcessStepLocalServiceUtil.findByG_SP_SNO(
	...
	@Override
	public Response getListContacts(
		HttpServletRequest request, HttpHeaders header, Company company,
		...
		try {
			...
			JSONObject jsonData = (JSONObject) actions.getContacts(
	...
	@Override
	public Response getByDeliverableState(
		HttpServletRequest request, HttpHeaders header, Company company,
		Locale locale, User user, ServiceContext serviceContext, Long id,
		String state) {
		...
		try {
			...
			List<DossierFile> dossierFileList =
				DossierFileLocalServiceUtil.getDossierFilesByDossierId(id);
			...
			//
			//
			List<Deliverable> deliverableList = action.getDeliverableByState(
				sb.toString(), Integer.valueOf(state));
