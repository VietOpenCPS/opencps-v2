			...
			<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();" validateOnBlur="<%= false %>">
				...
				<aui:fieldset>
					...
					if (authType.equals(CompanyConstants.AUTH_TYPE_EA)) {
						loginLabel = "\u0110\u1ECBa ch\u1EC9 email \u0111\u0103ng nh\u1EADp";
					}
					...
					<aui:input label="<%=passwordLabel %>" name="password" showRequiredLabel="<%= false %>" type="password" value="<%= password %>">
						<aui:validator name="required" />
					</aui:input>
				...
				<aui:button-row>
					<%
						...
						String submitLabel = "\u0110\u0103ng nh\u1EADp";
					
					%>
					<aui:button type="submit" value="<%=submitLabel %>" style="color: #fff;" />
				</aui:button-row>
			</aui:form>
