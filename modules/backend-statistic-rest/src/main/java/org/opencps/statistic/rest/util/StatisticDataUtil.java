...
public class StatisticDataUtil {
	public static ServiceDomainResponse getLocalServiceDomain(ServiceDomainRequest payload) {
		...
		try {
			...
			sorts = new Sort[] {
				SortFactoryUtil.create(DictItemTerm.SIBLING_SEARCH + DossierTerm.NUMBER_SORT_ABLE, Sort.INT_TYPE, false) };
	...
	public static GetPersonResponse getLocalPersonResponse(GetPersonRequest payload) {
		...
		try {
			...
			Sort[] sorts = new Sort[] { SortFactoryUtil.create(DossierTerm.TREE_INDEX_SORT_ABLE, Sort.STRING_TYPE, false) };
