...
public class VotingResultIndexer extends BaseIndexer<VotingResult> {
	...
	public void postProcessSearchQuery(
		...
		LinkedHashMap<String, Object> params =
			(LinkedHashMap<String, Object>) searchContext.getAttribute(
				VotingResultTerm.PARAMS);

		if (params != null) {
			String expandoAttributes =
				(String) params.get(VotingResultTerm.EXPANDO_ATTRIBUTES);
	...
	@Override
	protected void doDelete(VotingResult VotingResult)
		throws Exception {

		deleteDocument(
			VotingResult.getCompanyId(), VotingResult.getVotingResultId());
	}

	@Override
	protected Document doGetDocument(VotingResult VotingResult)
		throws Exception {

		Document document = getBaseModelDocument(CLASS_NAME, VotingResult);

		document.addNumberSortable(
			VotingResultTerm.VOTING_RESULT_ID,
			VotingResult.getVotingResultId());
		document.addKeywordSortable(
			Field.COMPANY_ID, String.valueOf(VotingResult.getCompanyId()));
		document.addDateSortable(
			Field.CREATE_DATE, VotingResult.getCreateDate());
		document.addDateSortable(
			Field.MODIFIED_DATE, VotingResult.getModifiedDate());
		document.addKeywordSortable(
			Field.USER_ID, String.valueOf(VotingResult.getUserId()));
		document.addKeywordSortable(
			Field.USER_NAME, String.valueOf(VotingResult.getUserName()));
		document.addNumberSortable(Field.GROUP_ID, VotingResult.getGroupId());

		document.addNumberSortable(
			VotingResultTerm.VOTING_ID, VotingResult.getVotingId());
		document.addTextSortable(
			VotingResultTerm.FULLNAME, VotingResult.getFullname());
		document.addTextSortable(
			VotingResultTerm.EMAIL, VotingResult.getEmail());
		document.addTextSortable(
			VotingResultTerm.COMMENT, VotingResult.getComment());
		document.addTextSortable(
			VotingResultTerm.SELECTED, VotingResult.getSelected());
		...
		if (votingId > 0) {
			...
			if (voting != null) {
				...
				document.addTextSortable(
					VotingTerm.CLASS_PK, voting.getClassPK());
				document.addTextSortable(
					VotingTerm.VOTING_CODE, voting.getVotingCode());
				document.addTextSortable(
					VotingTerm.VOTING_SUBJECT, voting.getSubject());
				...
				if (classPK > 0) {
					...
					if (ConfigProps.get(
						ConfigConstants.VOTING_CLASSNAME_EMPLOYEE).equals(
							className)) {
						Employee employee =
							EmployeeLocalServiceUtil.fetchEmployee(classPK);
						if (employee != null) {
							List<Dossier> dossierList =
								DossierLocalServiceUtil.findByGID(
									employee.getGroupId(), 0, 2);
							if (dossierList != null && dossierList.size() > 0) {
								Dossier dossier = dossierList.get(0);
								if (dossier != null) {
									document.addTextSortable(
										DossierTerm.GOV_AGENCY_CODE,
										dossier.getGovAgencyCode());
									document.addTextSortable(
										DossierTerm.GOV_AGENCY_NAME,
					...
					else if (ConfigProps.get(
						ConfigConstants.VOTING_CLASSNAME_DOSSIER).equals(
							className)) {
						Dossier dossier =
							DossierLocalServiceUtil.fetchDossier(classPK);
						if (dossier != null) {
							document.addTextSortable(
								DossierTerm.GOV_AGENCY_CODE,
								dossier.getGovAgencyCode());
							...
							ServiceInfo serviceInfo =
								ServiceInfoLocalServiceUtil.getByCode(
									dossier.getGroupId(),
									dossier.getServiceCode());
							if (serviceInfo != null) {
								document.addTextSortable(
									DossierTerm.DOMAIN_CODE,
									serviceInfo.getDomainCode());
								document.addTextSortable(
									DossierTerm.DOMAIN_NAME,
									serviceInfo.getDomainName());
							}
						}
					}
				}
	...
	@Override
	protected void doReindex(VotingResult VotingResult)
		throws Exception {

		Document document = getDocument(VotingResult);

		IndexWriterHelperUtil.updateDocument(
			getSearchEngineId(), VotingResult.getCompanyId(), document,
			isCommitImmediately());
	}
	...
	protected void doReindex(String className, long classPK)
		...
		VotingResult VotingResult =
			VotingResultLocalServiceUtil.fetchVotingResult(classPK);
	...
	protected void reindexMVotingResult(long companyId)
		throws PortalException {

		final IndexableActionableDynamicQuery indexableActionableDynamicQuery =
			VotingResultLocalServiceUtil.getIndexableActionableDynamicQuery();

		indexableActionableDynamicQuery.setCompanyId(companyId);
		indexableActionableDynamicQuery.setPerformActionMethod(
			new ActionableDynamicQuery.PerformActionMethod<VotingResult>() {
	...
	private static final Log _log =
		LogFactoryUtil.getLog(VotingResultIndexer.class);
