...
public class VotingIndexer extends BaseIndexer<Voting> {
	...
	public void postProcessSearchQuery(
		...
		if (params != null) {
			String expandoAttributes =
				(String) params.get(VotingTerm.EXPANDO_ATTRIBUTES);
	...
	@Override
	protected void doDelete(Voting voting)
		throws Exception {
	...
	@Override
	protected Document doGetDocument(Voting voting)
		throws Exception {
		...
		document.addKeywordSortable(
			Field.COMPANY_ID, String.valueOf(voting.getCompanyId()));
		...
		document.addKeywordSortable(
			Field.USER_ID, String.valueOf(voting.getUserId()));
		document.addKeywordSortable(
			Field.USER_NAME, String.valueOf(voting.getUserName()));
		document.addNumberSortable(Field.GROUP_ID, voting.getGroupId());

		User user = UserLocalServiceUtil.fetchUser(voting.getUserId());

		document.addTextSortable(
			VotingTerm.EMAIL, Validator.isNotNull(user)
		...
		document.addTextSortable(
			VotingTerm.TEMPLATE_NO, voting.getTemplateNo());
		document.addTextSortable(
			VotingTerm.COMMENTABLE, String.valueOf(voting.getCommentable()));
		document.addTextSortable(
	...
	@Override
	protected void doReindex(Voting voting)
		throws Exception {

		Document document = getDocument(voting);

		IndexWriterHelperUtil.updateDocument(
			getSearchEngineId(), voting.getCompanyId(), document,
			isCommitImmediately());
	}
	...
	protected void reindexMVoting(long companyId)
		throws PortalException {

		final IndexableActionableDynamicQuery indexableActionableDynamicQuery =
			VotingLocalServiceUtil.getIndexableActionableDynamicQuery();

		indexableActionableDynamicQuery.setCompanyId(companyId);
		indexableActionableDynamicQuery.setPerformActionMethod(
			new ActionableDynamicQuery.PerformActionMethod<Voting>() {
