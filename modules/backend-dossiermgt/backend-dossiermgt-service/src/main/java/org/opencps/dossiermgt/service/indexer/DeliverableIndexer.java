...
public class DeliverableIndexer extends BaseIndexer<Deliverable> {
	...
	@Override
	protected void doDelete(Deliverable object)
		throws Exception {
	...
	@Override
	protected Document doGetDocument(Deliverable object)
		throws Exception {
		...
		document.addKeywordSortable(
			Field.USER_NAME, String.valueOf(object.getUserName()));
		document.addKeywordSortable(Field.ENTRY_CLASS_NAME, CLASS_NAME);
		document.addNumberSortable(
			Field.ENTRY_CLASS_PK, object.getPrimaryKey());

		// add number fields
		document.addNumberSortable(
			DeliverableTerm.DELIVERABLE_ID, object.getDeliverableId());
		if (Validator.isNotNull(object.getIssueDate())) {
			document.addDateSortable(
				DeliverableTerm.ISSUE_DATE, object.getIssueDate());
		}
		else {
			document.addTextSortable(
				DeliverableTerm.ISSUE_DATE, StringPool.BLANK);
		}
		if (Validator.isNotNull(object.getExpireDate())) {
			document.addDateSortable(
				DeliverableTerm.EXPIRE_DATE, object.getExpireDate());
		}
		else {
			document.addTextSortable(
				DeliverableTerm.EXPIRE_DATE, StringPool.BLANK);
		}

		if (Validator.isNotNull(object.getRevalidate())) {
			document.addDateSortable(
				DeliverableTerm.REVALIDATE, object.getRevalidate());
		}
		else {
			document.addTextSortable(
				DeliverableTerm.REVALIDATE, StringPool.BLANK);
		}
		...
		if (Validator.isNotNull(deliverableCode)) {
			document.addTextSortable(
				DeliverableTerm.DELIVERABLE_CODE, deliverableCode);
			document.addTextSortable(
			...
			document.addTextSortable(
				DeliverableTerm.DELIVERABLE_CODE, StringPool.BLANK);
			document.addTextSortable(
				DeliverableTerm.DELIVERABLE_CODE_SEARCH, StringPool.BLANK);
		}

		document.addTextSortable(
			DeliverableTerm.DELIVERABLE_NAME, object.getDeliverableName());
		document.addTextSortable(
			DeliverableTerm.DELIVERABLE_TYPE, object.getDeliverableType());
		document.addTextSortable(
			DeliverableTerm.GOV_AGENCY_CODE, object.getGovAgencyCode());
		document.addTextSortable(
			DeliverableTerm.GOV_AGENCY_NAME, object.getGovAgencyName());
		document.addTextSortable(
			DeliverableTerm.APPLICANT_ID_NO, object.getApplicantIdNo());
		document.addTextSortable(
			DeliverableTerm.APPLICANT_NAME, object.getApplicantName());
		document.addTextSortable(DeliverableTerm.SUBJECT, object.getSubject());
		document.addTextSortable(
			DeliverableTerm.FORM_DATA, object.getFormData());

		document.addNumberSortable(
			ModelKeysDeliverable.ISSUEDATE,
			Validator.isNotNull(object.getIssueDate())
				? object.getIssueDate().getTime() : null);
		document.addNumberSortable(
			ModelKeysDeliverable.EXPIREDATE,
			Validator.isNotNull(object.getExpireDate())
				? object.getExpireDate().getTime() : null);
		document.addNumberSortable(
			ModelKeysDeliverable.REVALIDATE,
			Validator.isNotNull(object.getRevalidate())
		...
		document.addNumberSortable(
			ModelKeysDeliverable.FILEENTRYID, object.getFileEntryId());
		document.addNumberSortable(
			ModelKeysDeliverable.DOCSYNC, object.getDocSync());
		document.addNumberSortable(
			ModelKeysDeliverable.DOSSIERID, object.getDossierId());

		document.addTextSortable(
			DeliverableTerm.FILE_ATTACHS, object.getFileAttachs());
		...
		document.addTextSortable(
			DeliverableTerm.FORM_SCRIPT, object.getFormScript());
		document.addTextSortable(
			DeliverableTerm.FORM_REPORT, object.getFormReport());
		document.addNumberSortable(
			DeliverableTerm.DELIVERABLE_STATE, object.getDeliverableState());
	...
	@Override
	protected void doReindex(Deliverable object)
		throws Exception {
	...
	protected void reindex(long companyId)
		...
		final IndexableActionableDynamicQuery indexableActionableDynamicQuery =
			DeliverableLocalServiceUtil.getIndexableActionableDynamicQuery();

		indexableActionableDynamicQuery.setCompanyId(companyId);
		indexableActionableDynamicQuery.setPerformActionMethod(
			new ActionableDynamicQuery.PerformActionMethod<Deliverable>() {
	...
	protected List<Object[]> getKeyValues(
		String formData, List<Object[]> keyValues) {
		...
		if (json != null) {
			...
			while (itr.hasNext()) {
				...
				try {
					...
					parseJSONObjectIndex(
						keyValues, json.getJSONObject(key), key);
				}
		...
		if (json != null) {
			...
			while (itr.hasNext()) {
				...
				try {
					...
					keyValues.add(keyValue);
					parseJSONObjectIndex(
						keyValues, json.getJSONObject(key),
						keyValue[0].toString());
				}
				catch (JSONException e) {
					_log.error(e);
