...
//		_log.info("auto =: " + auto);
		...
		for (Role r : userRolesAdminCheck) {
			if (ConstantUtils.ADMINISTRATOR.equalsIgnoreCase(r.getName())) {
...
//							_log.info("SONDT NEXTACTIONLIST PRECONDITION ======== " + preCondition);
	...
	@Override
	public JSONObject getDetailNextActions(long userId,long companyId,long groupId,LinkedHashMap<String, Object> params,
		Sort[] sorts,Integer start,Integer end,ServiceContext serviceContext)
	{
		JSONObject result = JSONFactoryUtil.createJSONObject();
		...
		try
		{
			...
			if (dossier != null)
			{
					...
					else
					{
						if (Validator.isNotNull(paymentFeeData))
						{
							...
							if (jsonPaymentFee != null)
							{
								...
								payment.put(PaymentFileTerm.PAYMENT_AMOUNT,jsonPaymentFee.has(PaymentFileTerm.PAYMENT_AMOUNT) ?
				...
				ProcessStep processStep = ProcessStepLocalServiceUtil.fetchBySC_GID(postStepCode,groupId,serviceProcessId);
				if (processStep != null)
				{
					...
					if (Validator.isNotNull(processStep.getRoleAsStep()))
					{
						...
						for (String stepCode : steps)
						{
							...
							{
								lstUser
									.addAll(processRoleAsStepListUser(dossier,stepCode,serviceProcessId,processStep,processStepRoleList));
							}
						}
					}
					else
				...
				if (processAction != null)
				{
					...
					receivingObj.put(DossierTerm.RECEIVE_DATE,
						dossier.getReceiveDate() != null ? dossier.getReceiveDate().getTime() : receiveDate.getTime());
					Date dueDate = null;
					...
					else
					{
						...
						if (Validator.isNotNull(String.valueOf(durationCount)) && durationCount > 0d)
						{
							...
							DueDateUtils dueDateUtils = new DueDateUtils(new Date(),durationCount,serviceProcess.getDurationUnit(),
								groupId);
					...
					if (postStep != null)
					{
						if (ReadFilePropertiesUtils.get(ConstantUtils.STATUS_NEW)
							.equals(postStep.getDossierStatus()) || DossierTerm.DOSSIER_STATUS_PROCESSING
							.equals(postStep.getDossierStatus()) && dossier.getReceiveDate() == null)
						{
				...
				List<DossierPart> partList = DossierPartLocalServiceUtil
					.getByTemplateNo(groupId,dossierTemplate.getTemplateNo());
				...
				if (returnFileTempNoList != null && !returnFileTempNoList.isEmpty())
				{
					...
					if (partList != null && partList.size() > 0)
					{
						for (DossierPart dossierPart : partList)
						{
							...
							{
								List<DossierFile> fileList = DossierFileLocalServiceUtil
									.getDossierFileByDID_FTNO_DPT(dossierId,fileTemplateNo,dossierPartType,false);
					...
					result.put(DossierActionTerm.RETURN_FILES,returnFiles);
				}
					...
					if (partList != null && partList.size() > 0)
					{
						...
						for (DossierPart dossierPart : partList)
						{
							...
							if (createFileTempNoList.contains(fileTemplateNo))
							{
								...
								//Get dossierMark
								DossierMark dossierMark = DossierMarkLocalServiceUtil
									.getDossierMarkbyDossierId(groupId,dossierId,dossierPart.getPartNo());
								if (dossierMark != null)
								{
								...
								String formDataDeliverables = AutoFillFormData.sampleDataBinding(sampleData,dossier,context);
								...
								if (Validator.isNull(strDeliverableType))
								{
									List<DossierFile> dossierFilesResult = DossierFileLocalServiceUtil
										.getDossierFileByDID_FTNO_DPTS(dossierId,fileTemplateNo,
											new int[] {DossierPartTerm.DOSSIER_PART_TYPE_OUTPUT,
												DossierPartTerm.DOSSIER_PART_TYPE_GROUP_OUTPUT},false,QueryUtil.ALL_POS,QueryUtil.ALL_POS,
									...
									{
										createFile = processFileResult(dossierFilesResult,createFile,dossierPart.getPartNo());
										//										_log.debug("createFile: "+createFile.toJSONString());

									}
									else
									{
										createFile = processEFormByCreateFile(dossierPart,groupId,dossierId,sampleData,fileTemplateNo,
											dossierTempNo,createFile,serviceContext);
										dossierFilesResult = DossierFileLocalServiceUtil
											.getDossierFileByDID_FTNO_DPTS_NOT_NULL_FID(dossierId,fileTemplateNo,
												new int[] {DossierPartTerm.DOSSIER_PART_TYPE_OUTPUT,
													DossierPartTerm.DOSSIER_PART_TYPE_GROUP_OUTPUT},0,false);
									}
									...
									if (deliverableTypeObject != null)
									{
										...
										if (mappingDataObj.has(DeliverableTypesTerm.DELIVERABLES_KEY))
										{
											...
											if (Validator.isNull(deliverables))
											{
												...
												if (dossierFilesResult != null && !dossierFilesResult.isEmpty())
												{
													//													_log.info("--------DELIVERABLES DOSSIER FILE RESULT NOT NULL----------");
													createFile = processFileResult(dossierFilesResult,createFile,dossierPart.getPartNo());
												}
												else
												{
													...
													formData = AutoFillFormData
														.sampleDataBinding(dossierPart.getSampleData(),dossierId,serviceContext);
													...
													if (eForm)
													{
														createFile = processEFormAndUpdateDossierFile(dossierPart,groupId,dossierId,formData,
															fileTemplateNo,dossierTempNo,formDataObj,createFile,serviceContext);
													}
													else if (Validator.isNotNull(formData))
													{
														...
														if (mappingDataObj.has(DeliverableTypesTerm.DELIVERABLES_KEY))
														{
															...
															if (Validator.isNull(deliverables))
															{
																createFile = processEFormAndUpdateFormData(dossierPart,groupId,dossierId,formData,
																	fileTemplateNo,dossierTempNo,formDataObj,createFile,serviceContext);
															}
															else
															{
																if (formDataObj.has(deliverables))
																{
																	...
																	for (int i = 0; i < deliverablesArr.length(); i++)
																	{
																		...
																		createFile.put(DossierFileTerm.COUNTER,counter);
																		createFile.put(DeliverableTerm.DELIVERABLE_TYPE,deliverableTypeObject != null ?
																			deliverableTypeObject.getTypeCode() :
																			StringPool.BLANK);
																		createFiles.put(createFile);
																	}
																}
												...
												//												_log.info("--------DELIVERABLES IS NULL PUT DELIVERABLE TYPE----------" + deliverables);
												dossierFilesResult = DossierFileLocalServiceUtil
													.getDossierFileByDID_FTNO_DPTS_NOT_NULL_FID(dossierId,fileTemplateNo,
														new int[] {DossierPartTerm.DOSSIER_PART_TYPE_OUTPUT,
															DossierPartTerm.DOSSIER_PART_TYPE_GROUP_OUTPUT},0,false);
												...
												createFile.put(DeliverableTerm.DELIVERABLE_TYPE,
													deliverableTypeObject != null ? deliverableTypeObject.getTypeCode() : StringPool.BLANK);
												createFiles.put(createFile);
													...
													if (eForm && !dossierPart.getESign())
													{
														...
														try
														{
															dossierFile = DossierFileLocalServiceUtil
																.getDossierFileByDID_FTNO_DPTS_First(dossierId,fileTemplateNo,
																	DossierPartTerm.DOSSIER_PART_TYPE_OUTPUT,false,
																	new DossierFileComparator(false,Field.CREATE_DATE,Date.class));
														}
														catch (Exception e)
														{
														...
														try
														{
															dossierFile = DossierFileLocalServiceUtil
																.getDossierFileByDID_FTNO_DPTS_First(dossierId,fileTemplateNo,
																	DossierPartTerm.DOSSIER_PART_TYPE_GROUP_OUTPUT,false,
																	new DossierFileComparator(false,Field.CREATE_DATE,Date.class));
														}
														catch (Exception e)
														{
														...
														if (Validator.isNull(dossierFile))
														{
															...
															dossierFile = actions
																.addDossierFile(groupId,dossierId,StringPool.BLANK,dossier.getDossierTemplateNo(),
													...
													else if (Validator.isNotNull(formData))
													{
														...
														if (Validator.isNotNull(dossierPart.getDeliverableType()))
														{
															DeliverableType dlt = DeliverableTypeLocalServiceUtil
																.getByCode(groupId,dossierPart.getDeliverableType());
															if (dlt != null)
															{
																		...
																		if (formDataObj.has(deliverables))
																		{
																			...
																			for (int i = 0; i < deliverablesArr.length(); i++)
																			{
																				...
																				dossierFile = actions.addDossierFile(groupId,dossierId,StringPool.BLANK,
																					dossier.getDossierTemplateNo(),dossierPart.getPartNo(),fileTemplateNo,
																				...
																				docFileReferenceUid = dossierFile.getReferenceUid();
																				dossierFileId = dossierFile.getDossierFileId();
																				newFormDataObj.put("LicenceNo",DeliverableNumberGenerator
																					.generateDeliverableNumber(groupId,companyId,dlt.getDeliverableTypeId()));

																				DossierFileLocalServiceUtil
																					.updateFormData(groupId,dossierId,docFileReferenceUid,
																						newFormDataObj.toJSONString(),serviceContext);
																			}
																		}
																	}
				...
				result.put(ProcessActionTerm.CREATE_FILES,createFiles);
				//				_log.info("NEXT ACTION CREATE FILES: " + result.toString());
			}
		}
		catch (Exception e)
		{
	...
	@Override
	public JSONArray getPayloadNextActions(long userId,long companyId,long groupId,LinkedHashMap<String, Object> params,
		Sort[] sorts,Integer start,Integer end,ServiceContext serviceContext)
	{
		...
		try
		{
			...
			if (dossier != null)
			{
				...
				if (dossierAction != null)
				{
					String actionCode = params.containsKey(DossierActionTerm.ACTION_CODE) ?
						(String) params.get(DossierActionTerm.ACTION_CODE) :
						StringPool.BLANK;
	...
	private JSONObject processEFormAndUpdateFormData(DossierPart dossierPart,long groupId,long dossierId,String formData,
		String fileTemplateNo,String dossierTempNo,JSONObject formDataObj,JSONObject createFile,
		ServiceContext serviceContext)
	{
		...
		try
		{
			dossierFile = DossierFileLocalServiceUtil.getDossierFileByDID_FTNO_DPT_First(dossierId,fileTemplateNo,2,false,
				new DossierFileComparator(false,Field.CREATE_DATE,Date.class));
			...
			{
				dossierFile = actions
					.addDossierFile(groupId,dossierId,StringPool.BLANK,dossierTempNo,dossierPart.getPartNo(),fileTemplateNo,
						dossierPart.getPartName(),StringPool.BLANK,0L,null,StringPool.BLANK,String.valueOf(false),serviceContext);
				dossierFile.setFormScript(dossierPart.getFormScript());
				dossierFile.setEForm(dossierPart.getEForm());
				dossierFile = DossierFileLocalServiceUtil.updateDossierFile(dossierFile);
			}
	...
	private JSONObject processEFormAndUpdateDossierFile(DossierPart dossierPart,long groupId,long dossierId,
		String formData,String fileTemplateNo,String dossierTempNo,JSONObject formDataObj,JSONObject createFile,
		ServiceContext serviceContext)
	{
		...
		try
		{
			...
			if (dossierFile == null)
			{
				...
				dossierFile = actions
					.addDossierFile(groupId,dossierId,StringPool.BLANK,dossierTempNo,dossierPart.getPartNo(),fileTemplateNo,
						dossierPart.getPartName(),StringPool.BLANK,0L,null,StringPool.BLANK,String.valueOf(false),
						dossierPart.getFormScript(),dossierPart.getFormReport(),dossierPart.getEForm(),formData,serviceContext);
		...
		String fileTemplateNo,String dossierTempNo,JSONObject createFile,ServiceContext serviceContext)
	{
		...
		String formData = eForm ?
			AutoFillFormData.sampleDataBinding(sampleData,dossierId,serviceContext) :
			StringPool.BLANK;
		...
		Sort[] sorts,int start,int end,ServiceContext serviceContext) throws PortalException
	{
		...
		if (dossier != null)
		{
			...
			if (Validator.isNotNull(stepCode))
			{
				...
				try
				{
					lstProcessAction = ProcessActionLocalServiceUtil
					...
					for (ProcessAction processAction : lstProcessAction)
					{
						...
						ProcessStep processStep = ProcessStepLocalServiceUtil
							.fetchBySC_GID(postStepCode,groupId,processAction.getServiceProcessId());

						List<ProcessStepRole> lstProcessStepRole = ProcessStepRoleLocalServiceUtil
							.findByP_S_ID(processStep.getProcessStepId());
						List<User> lstUser = new ArrayList<User>();

						if (Validator.isNotNull(auto) && (DossierTerm.PRECONDITION_SUBMIT
							.equals(processAction.getAutoEvent()) || DossierTerm.PRECONDITION_TIMER
							.equals(processAction.getAutoEvent())))
						{
						}
						else
						{
								...
								for (ServiceProcessRole serviceProcessRole : serviceProcessRoles)
								{
									...
									if (users != null)
									{
										for (User user : users)
										{
											...
											Employee emp = EmployeeLocalServiceUtil
												.fetchByF_mappingUserId(dossier.getGroupId(),user.getUserId());

											if (!user.isLockout() && user.isActive() && Validator.isNotNull(emp) && emp
												.getWorkingStatus() == 1)
											{
	...
	private void generateCreateDossierFiles(long groupId,long userId,Dossier dossier,ProcessAction processAction)
		throws PortalException
	{
		ServiceContext context = new ServiceContext();
			...
			if (dossierParts != null)
			{
				for (DossierPart dossierPart : dossierParts)
				{
					...
					if (dossierFileTemplateNos.contains(fileTemplateNo))
					{
						...
						{
							formData = AutoFillFormData.sampleDataBinding(dossierPart.getSampleData(),dossier.getDossierId(),context);
							try
							{
								dossierFile = DossierFileLocalServiceUtil
									.getDossierFileByDID_FTNO_DPT_First(dossier.getDossierId(),fileTemplateNo,2,false,
										new DossierFileComparator(false,Field.CREATE_DATE,Date.class));
							}
							catch (Exception e)
							{
							...
							if (Validator.isNull(dossierFile) && dossierPart.getEForm())
							{
								dossierFile = actions
									.addDossierFile(groupId,dossier.getDossierId(),StringPool.BLANK,dossier.getDossierTemplateNo(),
							...
							if (deliverableTypeObject != null)
							{
								...
								if (mappingDataObj.has(DeliverableTypesTerm.DELIVERABLES_KEY))
								{
									...
									if (Validator.isNull(deliverables))
									{
										//Add one deliverable
										dossierFilesResult = DossierFileLocalServiceUtil
											.getDossierFileByDID_FTNO_DPT(dossier.getDossierId(),fileTemplateNo,2,false,QueryUtil.ALL_POS,
												QueryUtil.ALL_POS,new DossierFileComparator(false,Field.CREATE_DATE,Date.class));
										if (dossierFilesResult != null && !dossierFilesResult.isEmpty())
										{
										}
										else
										{
											formData = AutoFillFormData
												.sampleDataBinding(dossierPart.getSampleData(),dossier.getDossierId(),context);
											try
											{
												dossierFile = DossierFileLocalServiceUtil
													.getDossierFileByDID_FTNO_DPT_First(dossier.getDossierId(),fileTemplateNo,2,false,
											...
											if (Validator.isNull(dossierFile))
											{
												dossierFile = actions.addDossierFile(groupId,dossier.getDossierId(),StringPool.BLANK,
													dossier.getDossierTemplateNo(),dossierPart.getPartNo(),fileTemplateNo,
													dossierPart.getPartName(),StringPool.BLANK,0L,null,StringPool.BLANK,String.valueOf(false),
													context);
												dossierFile.setFormScript(dossierPart.getFormScript());
									...
									else
									{
										...
										if (mappingDataObj.has(DeliverableTypesTerm.DELIVERABLES_KEY))
										{
											...
											if (Validator.isNull(deliverables))
											{
												...
												actions.updateDossierFileFormData(groupId,dossier.getDossierId(),docFileReferenceUid,formData,
													context);
											}
											else
											{
												if (formDataObj.has(deliverables))
												{
													...
													for (int i = 0; i < deliverablesArr.length(); i++)
													{
														...
														docFileReferenceUid = dossierFile.getReferenceUid();
														actions.updateDossierFileFormData(groupId,dossier.getDossierId(),docFileReferenceUid,
	...
	public DossierAction doAction(long groupId,long userId,Dossier dossier,ProcessOption option,ProcessAction proAction,
		...
		ServiceContext context,ErrorMsgModel errorModel) throws Exception
	{
		return CPSDossierBusinessLocalServiceUtil
			.doAction(groupId,userId,dossier,option,proAction,actionCode,actionUser,actionNote,payload,assignUsers,payment,
				syncType,context);
	}
	...
	@Override
	public DossierAction doAction(long groupId,long dossierId,String referenceUid,String actionCode,long processActionId,
		String actionUser,String actionNote,long assignUserId,long userId,String subUsers,ServiceContext context)
		throws PortalException
	{
		...
		try
		{
			...
			option = getProcessOption(dossier.getServiceCode(),dossier.getGovAgencyCode(),dossier.getDossierTemplateNo(),
				groupId);
		}
		catch (Exception e)
		{
		...
		if (processAction != null && Validator.isNotNull(processAction.getPaymentFee()))
		{
			...
			{
				DossierPaymentUtils
					.processPaymentFile(processAction,processAction.getPaymentFee(),groupId,dossierId,userId,context,
		...
		if (actionCode.contentEquals(SPECIAL_ACTION) && (types.contains(OCPSUserUtils.APPLICANT_01) || types
			.contains(OCPSUserUtils.APPLICANT_02)))
		{
			...
			if (curStep != null && actionConfig != null)
			{
				...
				dossierAction = DossierActionLocalServiceUtil
					.updateDossierAction(groupId,0,dossierId,serviceProcessId,0l,fromStepCode,fromStepName,fromSequenceNo,
						...
						curStep.getStepName(),curStep.getSequenceNo(),null,0l,payload,curStep.getStepInstruction(),
						...
						(actionConfig.getEventType() == ActionConfigTerm.EVENT_TYPE_NOT_SENT ?
							...
							DossierActionTerm.EVENT_STATUS_WAIT_SENDING),context);
			...
			{
				dossierAction = DossierActionLocalServiceUtil
					.updateDossierAction(groupId,0,dossierId,serviceProcessId,0l,StringPool.BLANK,StringPool.BLANK,
						StringPool.BLANK,actionCode,actionUser,processAction.getActionName(),actionNote,actionOverdue,
						processAction.getSyncActionCode(),false,processAction.getRollbackable(),
						curStep != null ? curStep.getStepCode() : StringPool.BLANK,
						curStep != null ? curStep.getStepName() : StringPool.BLANK,
						curStep != null ? curStep.getSequenceNo() : StringPool.BLANK,null,0l,payload,
						curStep != null ? curStep.getStepInstruction() : StringPool.BLANK,DossierActionTerm.STATE_ALREADY_PROCESSED,
						DossierActionTerm.EVENT_STATUS_NOT_CREATED,context);
			}
			...
			if (Validator.isNotNull(subUsers))
			{
				...
				dossierActionUser
					.assignDossierActionUser(dossier,processAction.getAllowAssignUser(),dossierAction,userId,groupId,assignUserId,
						subUsersArray,0);
			}
			else
			{
				dossierActionUser
					.initDossierActionUser(processAction,dossier,processAction.getAllowAssignUser(),dossierAction,userId,groupId,
						assignUserId);
			}

			dossier = DossierLocalServiceUtil.updateStatus(groupId,dossierId,referenceUid,DossierStatusConstants.NEW,
				jsStatus.getString(DossierStatusConstants.NEW),StringPool.BLANK,StringPool.BLANK,StringPool.BLANK,
		...
		else
		{
			...
			dossierAction = DossierActionLocalServiceUtil
				.updateDossierAction(groupId,0,dossierId,serviceProcessId,0l,fromStepCode,fromStepName,fromSequenceNo,
					actionCode,actionUser,processAction.getActionName(),actionNote,actionOverdue,
					processAction.getSyncActionCode(),hasDossierSync,processAction.getRollbackable(),curStep.getStepCode(),
					curStep.getStepName(),curStep.getSequenceNo(),null,0l,payload,curStep.getStepInstruction(),state,eventStatus,
					context);
			...
			dossier = DossierLocalServiceUtil.updateStatus(groupId,dossierId,referenceUid,curStep.getDossierStatus(),
				...
				jsSubStatus.getString(curStep.getDossierSubStatus()),curStep.getLockState(),dossierAction.getStepInstruction(),
				context);
			...
			if (Validator.isNull(dossier.getDossierNo()) && (curStep.getDossierStatus()
				.contentEquals(DossierStatusConstants.PAYING) || (curStep.getDossierStatus()
				.contentEquals(DossierStatusConstants.PROCESSING))))
			{
				...
				String dossierRef = DossierNumberGenerator
					.generateDossierNumber(groupId,dossier.getCompanyId(),dossierId,option.getProcessOptionId(),
			...
			if (Validator.isNotNull(prvAction))
			{
				DossierActionLocalServiceUtil
					.updateNextActionId(prvAction.getDossierActionId(),dossierAction.getDossierActionId());
			}
			...
			if (Validator.isNotNull(autoEvent) && autoEvent.toLowerCase().contentEquals(DossierActionTerm.TIMMER))
			{
				flagEvent = true;
			}
			...
			if (Validator.isNotNull(preCondition))
			{
				...
				if (preCondition.toLowerCase().contentEquals(DossierActionTerm.REJECT_CANCELLING))
				{
					// flag-off
					...
					DossierRequestUDLocalServiceUtil
						.updateDossierRequest(0,dossierId,refUid,DossierActionTerm.REJECT_CANCELLING,actionNote,0,status,context);

					// in SERVER

					context.setScopeGroupId(sourceDossier.getGroupId());
					DossierRequestUDLocalServiceUtil
						.updateDossierRequest(0,sourceDossier.getDossierId(),refUid,DossierActionTerm.REJECT_CANCELLING,actionNote,
							0,status,context);
				...
				if (preCondition.toLowerCase().contentEquals(DossierActionTerm.CANCELLING))
				{
					// flag-off
					...
					DossierRequestUDLocalServiceUtil
						.updateDossierRequest(0,dossierId,refUid,DossierActionTerm.CANCELLING,actionNote,0,status,context);
					...
					if (sourceDossier != null)
					{
						context.setScopeGroupId(sourceDossier.getGroupId());
						DossierRequestUDLocalServiceUtil
							.updateDossierRequest(0,sourceDossier.getDossierId(),refUid,DossierActionTerm.CANCELLING,actionNote,0,
								status,context);
					}
				...
				if (preCondition.toLowerCase().contentEquals(DossierActionTerm.REJECT_SUBMITTING))
				{
					// flag-off
					...
					DossierRequestUDLocalServiceUtil
						.updateDossierRequest(0,dossierId,refUid,DossierActionTerm.REJECT_SUBMITTING,actionNote,0,status,context);

					// in SERVER

					context.setScopeGroupId(sourceDossier.getGroupId());
					DossierRequestUDLocalServiceUtil
						.updateDossierRequest(0,sourceDossier.getDossierId(),refUid,DossierActionTerm.REJECT_SUBMITTING,actionNote,
							0,status,context);
				...
				if (preCondition.toLowerCase().contentEquals(DossierActionTerm.SUBMITTING))
				{
					if (flagEvent)
					{
						// flag-off
						...
						DossierRequestUDLocalServiceUtil
							.updateDossierRequest(0,dossierId,refUid,DossierActionTerm.SUBMITTING,actionNote,0,status,context);
						...
						if (sourceDossier != null)
						{
							context.setScopeGroupId(sourceDossier.getGroupId());
							DossierRequestUDLocalServiceUtil
								.updateDossierRequest(0,sourceDossier.getDossierId(),refUid,DossierActionTerm.SUBMITTING,actionNote,0,
									status,context);
						}
				...
				if (preCondition.toLowerCase().contentEquals(DossierActionTerm.REJECT_CORRECTING))
				{
					// flag-off
					...
					DossierRequestUDLocalServiceUtil
						.updateDossierRequest(0,dossierId,refUid,DossierActionTerm.REJECT_CORRECTING,actionNote,0,status,context);

					// in SERVER

					context.setScopeGroupId(sourceDossier.getGroupId());
					DossierRequestUDLocalServiceUtil
						.updateDossierRequest(0,sourceDossier.getDossierId(),refUid,DossierActionTerm.REJECT_CORRECTING,actionNote,
							0,status,context);
				...
				if (preCondition.toLowerCase().contentEquals(DossierActionTerm.CORRECTING))
				{
					if (flagEvent)
					{
						// flag-off
						...
						// IN CLIENT
						DossierRequestUDLocalServiceUtil
							.updateDossierRequest(0,dossierId,refUid,DossierActionTerm.CORRECTING,actionNote,0,status,context);
						...
						if (sourceDossier != null)
						{
							context.setScopeGroupId(sourceDossier.getGroupId());
							DossierRequestUDLocalServiceUtil
								.updateDossierRequest(0,sourceDossier.getDossierId(),refUid,DossierActionTerm.CORRECTING,actionNote,0,
									status,context);
						}
						context.setScopeGroupId(dossier.getGroupId());
					}
					else
					{
						...
						// IN CLIENT
						DossierRequestUDLocalServiceUtil
							.updateDossierRequest(0,dossierId,refUid,"correcting",actionNote,0,status,context);
		...
		String dossierBriefNote = DossierContentGenerator
			.getBriefNote(groupId,dossierId,postStep != null ? postStep.getBriefNote() : StringPool.BLANK);
		...
		List<ProcessPlugin> plugins = ProcessPluginLocalServiceUtil
			.getProcessPlugins(serviceProcessId,curStep != null ? curStep.getStepCode() : StringPool.BLANK);
		...
		ServiceContext context)
	{
		...
		try
		{
			...
			DossierFile dossierFile = DossierFileLocalServiceUtil
				.getDossierFileByDID_FTNO_First(dossierId,fileTemplateNo,false,
					new DossierFileComparator(false,Field.CREATE_DATE,Date.class));
			...
			if (Validator.isNull(dossierFile))
			{
				...
				dossierFile = actions
					.addDossierFile(groupId,dossierId,PortalUUIDUtil.generate(),dossierTemplateNo,dossierPart.getPartNo(),
						fileTemplateNo,dossierPart.getPartName(),StringPool.BLANK,0L,null,StringPool.BLANK,String.valueOf(false),
						context);
				dossierFile.setEForm(dossierPart.getEForm());
				dossierFile.setFormScript(dossierPart.getFormScript());
				dossierFile = DossierFileLocalServiceUtil.updateDossierFile(dossierFile);
	...
	@Override public Dossier markerVisited(long groupId,long dossierId,String referenceUid) throws PortalException
	{
		// TODO Auto-generated method stub
		return null;
	}

	@Override public DossierAction doRollback(long groupId,long dossierId,String referenceUid,long userId)
		throws PortalException
	{
		// TODO Auto-generated method stub
		return null;
	...
	@Override public JSONObject getContacts(long groupId,long dossierId,String referenceUid) throws PortalException
	{
		// TODO Auto-generated method stub
		JSONObject result = JSONFactoryUtil.createJSONObject();
		// get DossierAction
	...
	private JSONObject getStatusText(long groupId,String collectionCode,String curStatus,String curSubStatus)
	{
		...
		if (Validator.isNotNull(dc) && Validator.isNotNull(curStatus))
		{
			...
			if (Validator.isNotNull(it))
			{
				...
				{
					DictItem dItem = DictItemLocalServiceUtil.fetchByF_dictItemCode(curSubStatus,dc.getPrimaryKey(),groupId);
		...
		long groupId) throws PortalException
	{
		...
		return ProcessOptionLocalServiceUtil.getByDTPLNoAndServiceCF(groupId,dossierTemplateNo,config.getServiceConfigId());
	}
	...
	protected boolean hasCreateDossier(long groupId,long dossierId,String refId,String actionCode,long serviceProcessId,
		boolean hasDossierSync) throws PortalException
	{
		...
		if (hasDossierSync && dossier.getDossierStatus().contentEquals(DossierStatusConstants.NEW) && !dossier.isOnline())
		{
			isCreate = true;
		}
	...
	protected boolean hasDossierSync(long groupId,long dossierId,String refId,ProcessAction action) throws PortalException
	{
	...
	protected boolean forcedDossierSync(long groupId,long dossierId,String refId,ProcessAction action,boolean isSubmit)
		throws PortalException
	{
		...
		if (!isSubmit && dossier.getOnline() && (action.getSyncActionCode().length() != 0 && action.getSyncActionCode()
			.contains(StringPool.EXCLAMATION)))
		{
			isSync = true;
		}
		...
		long serviceProcessId) throws PortalException
	{
		...
		try
		{
			...
			String dossierSubStatus = Validator.isNull(dossier.getDossierSubStatus()) ?
				StringPool.BLANK :
			...
			for (ProcessAction act : actions)
			{
				...
				{
					subStepStatus = Validator.isNull(step.getDossierSubStatus()) ? StringPool.BLANK : step.getDossierSubStatus();
				}
				...
				else
				{
					...
					if (step.getDossierStatus().contentEquals(dossierStatus) && subStepStatus.contentEquals(dossierSubStatus))
					{
		...
		catch (Exception e)
		{
			...
			throw new NotFoundException(
				"ProcessActionNotFoundException with actionCode= " + actionCode + "|serviceProcessId= " + serviceProcessId + "|referenceUid= " + refId + "|groupId= " + groupId);
		}

		return action;
	}

	@Override public Dossier initDossier(long groupId,long dossierId,String referenceUid,int counter,String serviceCode,
		String serviceName,String govAgencyCode,String govAgencyName,String applicantName,String applicantIdType,
		...
		String dossierTemplateNo,String password,int viaPostal,String postalAddress,String postalCityCode,
		String postalCityName,String postalDistrictCode,String postalDistrictName,String postalTelNo,boolean online,
		boolean notification,String applicantNote,int originality,ServiceContext context) throws PortalException
	{
		...
		try
		{
			...
			dossier = DossierLocalServiceUtil
				.initDossier(groupId,dossierId,referenceUid,counter,serviceCode,serviceName,govAgencyCode,govAgencyName,
					applicantName,applicantIdType,applicantIdNo,appIdDate,address,cityCode,cityName,districtCode,districtName,
					wardCode,wardName,contactName,contactTelNo,contactEmail,dossierTemplateNo,password,viaPostal,postalAddress,
					postalCityCode,postalCityName,postalDistrictCode,postalDistrictName,postalTelNo,online,notification,
	...
	public Dossier assignDossierToProcess(long dossierId,String dossierNote,String submissionNote,String briefNote,
		String dossierNo,long folderId,long dossierActionId,String serverNo,ServiceContext context) throws PortalException
	{

		return DossierLocalServiceUtil
			.assignToProcess(dossierId,dossierNote,submissionNote,briefNote,dossierNo,folderId,dossierActionId,serverNo,
				context);
	}
	...
	protected DossierAction getDossierAction(long dossierId,String actionCode)
	{
		...
		DossierAction dossierAction = DossierActionLocalServiceUtil
			.getDossierActionbyDossierIdandActionCode(dossierId,actionCode);
	...
	@Override public Dossier cloneDossier(long groupId,long dossierId,ServiceContext context) throws PortalException
	{
		...
		DossierFileLocalServiceUtil.cloneDossierFilesByDossierId(groupId,desDossier.getPrimaryKey(),dossierId,1,context);
			...
			if (Validator.isNotNull(statusCode) || Validator.isNotNull(subStatusCode))
			{
				...
				if (Validator.isNotNull(statusCode))
				{
					dictItem = DictItemLocalServiceUtil
						.fetchByF_dictItemCode(statusCode,dictCollection.getDictCollectionId(),groupId);
				}
				else
				{
					dictItem = DictItemLocalServiceUtil
						.fetchByF_dictItemCode(subStatusCode,dictCollection.getDictCollectionId(),groupId);
				}
	...
	@Override public JSONObject getDossierTodoPermission(long userId,long companyId,long groupId,
	...
	@Override public JSONObject getDossierCountTodoPermission(long userId,long companyId,long groupId,
				...
				if (statusCodeArr != null && statusCodeArr.length > 0)
				{
					for (String strStatus : statusCodeArr)
					{
						if (Validator.isNotNull(strStatus))
						{
							...
							if (!DossierTerm.DOSSIER_STATUS_DONE.equals(strStatus) && !DossierTerm.DOSSIER_STATUS_CANCELLED
								.equals(strStatus))
							{
	...
	@Override public Dossier createDossier(long groupId,String serviceCode,String govAgencyCode,String applicantName,
		String applicantIdType,String applicantIdNo,Date applicantIdDate,String address,String cityCode,String districtCode,
		String wardCode,String contactName,String contactTelNo,String contactEmail,boolean isSameAsApplicant,
		String delegateName,String delegateIdNo,String delegateTelNo,String delegateEmail,String delegateAddress,
		String delegateCityCode,String delegateDistrictCode,String delegateWardCode,String applicantNote,String briefNote,
		String dossierNo,String dossierTemplateNo,int viaPostal,String postalServiceCode,String postalServiceName,
		String postalAddress,String postalCityCode,String postalDistrictCode,String postalWardCode,String postalTelNo,
		int originality,ServiceContext context) throws PortalException
	{

		return DossierLocalServiceUtil
			.createDossier(groupId,serviceCode,govAgencyCode,applicantName,applicantIdType,applicantIdNo,applicantIdDate,
				address,cityCode,districtCode,wardCode,contactName,contactTelNo,contactEmail,isSameAsApplicant,delegateName,
				delegateIdNo,delegateTelNo,delegateEmail,delegateAddress,delegateCityCode,delegateDistrictCode,delegateWardCode,
				applicantNote,briefNote,dossierNo,dossierTemplateNo,viaPostal,postalServiceCode,postalServiceName,postalAddress,
	...
	{
		if (AUTO_EVENT_SUBMIT.equals(autoEvent) || AUTO_EVENT_TIMMER.equals(autoEvent) || AUTO_EVENT_LISTENER
			.equals(autoEvent) || AUTO_EVENT_SPECIAL.equals(autoEvent))
		{
	...
	private List<User> processRoleListUser(Dossier dossier,List<ProcessStepRole> processStepRoleList,
			...
			for (ProcessStepRole processStepRole : processStepRoleList)
			{
				...
				if (users != null && users.size() > 0)
				{
					...
					for (User user : users)
					{
						...
						if (!user.isLockout() && user.isActive() && Validator.isNotNull(emp) && emp.getWorkingStatus() == 1)
						{
	...
	private List<User> processRoleAsStepListUser(Dossier dossier,String stepCode,long serviceProcessId,
		ProcessStep processStep,List<ProcessStepRole> processStepRoleList)
	{
		List<User> lstUser = null;
		// Check roles
		lstUser = new ArrayList<User>();
		...
		for (ProcessStepRole role : processStepRoleList)
		{
			...
			for (User u : lstUsers)
			{
				...
				if (!u.isLockout() && u.isActive() && Validator.isNotNull(emp) && emp.getWorkingStatus() == 1)
				{
		...
		ProcessStep stepRoleAsStep = ProcessStepLocalServiceUtil
			.fetchBySC_GID(stepCode,dossier.getGroupId(),serviceProcessId);
		...
		ProcessStep processStep,List<ProcessStepRole> processStepRoleList)
	{
		...
		List<DossierActionUser> lstDaus = DossierActionUserLocalServiceUtil
			.getByDossierAndStepCode(dossier.getDossierId(),stepCode);
	...
	@Override public Dossier initUpdateDossier(long groupId,long id,String applicantName,String applicantIdType,
		String applicantIdNo,String applicantIdDate,String address,String cityCode,String cityName,String districtCode,
		String districtName,String wardCode,String wardName,String contactName,String contactTelNo,String contactEmail,
		...
		Integer vnpostalStatus,String vnpostalProfile,Integer fromViaPostal,ServiceContext serviceContext)
	{

		try
		{
			return DossierLocalServiceUtil
				.initUpdateDossier(groupId,id,applicantName,applicantIdType,applicantIdNo,applicantIdDate,address,cityCode,
					cityName,districtCode,districtName,wardCode,wardName,contactName,contactTelNo,contactEmail,dossierTemplateNo,
					viaPostal,postalAddress,postalCityCode,postalCityName,postalDistrictCode,postalDistrictName,postalTelNo,
					applicantNote,isSameAsApplicant,delegateName,delegateIdNo,delegateTelNo,delegateEmail,delegateAddress,
					delegateCityCode,delegateDistrictCode,delegateWardCode,sampleCount,vnpostalStatus,vnpostalProfile,
	...
	//		params.put(CInvoiceTerm.secret, "1");
	//		params.put(CInvoiceTerm.soid, "0");
	...
	@Override public List<User> getAssignUsersByStep(long userId,Dossier dossier,ProcessStep ps)
	{
		List<User> lstUser = new ArrayList<>();
		...
		if (ps != null)
		{
			...
			List<ProcessStepRole> processStepRoleList = ProcessStepRoleLocalServiceUtil.findByP_S_ID(ps.getProcessStepId());
			if (Validator.isNotNull(ps.getRoleAsStep()))
			{
				...
				for (String stepCode : steps)
				{
					if (stepCode.startsWith(StringPool.EXCLAMATION))
					{
						...
						String stepCodePunc = stepCode.substring(index + 1);
						lstUser.addAll(
							processRoleAsStepDonedListUser(dossier,stepCodePunc,ps.getServiceProcessId(),ps,processStepRoleList));
					}
					else
					{
	...
	private void updateDossierPayload(Dossier dossier,JSONObject obj)
	{
		...
		if (obj.has(DossierTerm.EXTEND_DATE) && Validator.isNotNull(obj.get(DossierTerm.EXTEND_DATE)) && GetterUtil
			.getLong(obj.get(DossierTerm.EXTEND_DATE)) > 0)
		{
		...
		if (obj.has(DossierTerm.DOSSIER_NO))
		{
			...
			if (Validator.isNotNull(obj.getString(DossierTerm.DOSSIER_NO)) && !obj.getString(DossierTerm.DOSSIER_NO)
				.equals(dossier.getDossierNo()))
			{
				//_log.info("Sync set dossier no");
				dossier.setDossierNo(obj.getString(DossierTerm.DOSSIER_NO));
			}
		}
		if (obj.has(DossierTerm.DUE_DATE) && Validator.isNotNull(obj.get(DossierTerm.DUE_DATE)) && GetterUtil
			.getLong(obj.get(DossierTerm.DUE_DATE)) > 0)
		{
		...
		if (obj.has(DossierTerm.FINISH_DATE) && Validator.isNotNull(obj.get(DossierTerm.FINISH_DATE)) && GetterUtil
			.getLong(obj.get(DossierTerm.FINISH_DATE)) > 0)
		{
		...
		if (obj.has(DossierTerm.RECEIVE_DATE) && Validator.isNotNull(obj.get(DossierTerm.RECEIVE_DATE)) && GetterUtil
			.getLong(obj.get(DossierTerm.RECEIVE_DATE)) > 0)
		{
			if (dossier.getReceiveDate() == null || obj.getLong(DossierTerm.RECEIVE_DATE) != dossier.getReceiveDate()
				.getTime())
			{
				dossier.setReceiveDate(new Date(obj.getLong(DossierTerm.RECEIVE_DATE)));
			}
		}
		if (obj.has(DossierTerm.SUBMIT_DATE) && Validator.isNotNull(obj.get(DossierTerm.SUBMIT_DATE)) && GetterUtil
			.getLong(obj.get(DossierTerm.SUBMIT_DATE)) > 0)
		{
			if (dossier.getSubmitDate() == null || (dossier.getSubmitDate() != null && obj
				.getLong(DossierTerm.SUBMIT_DATE) != dossier.getSubmitDate().getTime()))
			{
		...
		if (obj.has(DossierTerm.RELEASE_DATE) && Validator.isNotNull(obj.get(DossierTerm.RELEASE_DATE)) && GetterUtil
			.getLong(obj.get(DossierTerm.RELEASE_DATE)) > 0)
		{
			if (dossier.getReleaseDate() == null || obj.getLong(DossierTerm.RELEASE_DATE) != dossier.getReleaseDate()
				.getTime())
			{
	...
	@Override public ProcessOption getProcessOption(long serviceProcessId,long dossierTemplateId)
	{
	...
	@Override public Dossier initDossier(long groupId,long dossierId,String referenceUid,int counter,String serviceCode,
		...
		String dossierTemplateNo,String password,int viaPostal,String postalAddress,String postalCityCode,
		String postalCityName,String postalDistrictCode,String postalDistrictName,String postalTelNo,boolean online,
		boolean notification,String applicantNote,int originality,ServiceInfo service,ServiceProcess serviceProcess,
		ProcessOption processOption,ServiceContext context) throws PortalException
	{
		...
		try
		{
			...
			dossier = DossierLocalServiceUtil
				.initDossier(groupId,dossierId,referenceUid,counter,serviceCode,serviceName,govAgencyCode,govAgencyName,
					...
					wardCode,wardName,contactName,contactTelNo,contactEmail,dossierTemplateNo,password,viaPostal,postalAddress,
					postalCityCode,postalCityName,postalDistrictCode,postalDistrictName,postalTelNo,online,notification,
					applicantNote,originality,service,serviceProcess,processOption,context);
		...
		try
		{
			return DossierLocalServiceUtil
				.initUpdateDossier(groupId,id,applicantName,applicantIdType,applicantIdNo,applicantIdDate,address,cityCode,
					...
					viaPostal,postalAddress,postalCityCode,postalCityName,postalDistrictCode,postalDistrictName,postalTelNo,
					applicantNote,isSameAsApplicant,delegateName,delegateIdNo,delegateTelNo,delegateEmail,delegateAddress,
					delegateCityCode,delegateDistrictCode,delegateWardCode,sampleCount,dossierName,vnpostalStatus,vnpostalProfile,
					fromViaPostal,serviceContext);
		...
		try
		{
			return DossierLocalServiceUtil
				.initUpdateDossier(groupId,id,applicantName,applicantIdType,applicantIdNo,applicantIdDate,address,cityCode,
					...
					viaPostal,postalAddress,postalCityCode,postalCityName,postalDistrictCode,postalDistrictName,postalTelNo,
					applicantNote,isSameAsApplicant,delegateName,delegateIdNo,delegateTelNo,delegateEmail,delegateAddress,
					delegateCityCode,delegateDistrictCode,delegateWardCode,sampleCount,dossierName,briefNote,serviceContext);
		...
		Integer vnpostalStatus,String vnpostalProfile,Integer fromViaPostal,String formMeta,ServiceContext serviceContext)
	{
		try
		{
			if (Validator.isNotNull(formMeta) && JSONFactoryUtil.createJSONObject(formMeta) != null)
			{
				...
				return DossierLocalServiceUtil
					.initUpdateDossierMeta(groupId,id,applicantName,applicantIdType,applicantIdNo,applicantIdDate,address,
						cityCode,cityName,districtCode,districtName,wardCode,wardName,contactName,contactTelNo,contactEmail,
						dossierTemplateNo,viaPostal,postalAddress,postalCityCode,postalCityName,postalDistrictCode,
						postalDistrictName,postalTelNo,applicantNote,isSameAsApplicant,delegateName,delegateIdNo,delegateTelNo,
						delegateEmail,delegateAddress,delegateCityCode,delegateDistrictCode,delegateWardCode,sampleCount,
						dossierName,briefNote,delegateType,documentNo,documentDate,systemId,vnpostalStatus,vnpostalProfile,
						fromViaPostal,metaData,serviceContext);
			}
			else
			{
				return DossierLocalServiceUtil
					.initUpdateDossierFull(groupId,id,applicantName,applicantIdType,applicantIdNo,applicantIdDate,address,
						cityCode,cityName,districtCode,districtName,wardCode,wardName,contactName,contactTelNo,contactEmail,
						dossierTemplateNo,viaPostal,postalAddress,postalCityCode,postalCityName,postalDistrictCode,
						postalDistrictName,postalTelNo,applicantNote,isSameAsApplicant,delegateName,delegateIdNo,delegateTelNo,
						delegateEmail,delegateAddress,delegateCityCode,delegateDistrictCode,delegateWardCode,sampleCount,
	...
	public Dossier publishDossier(long groupId,long dossierId,String referenceUid,int counter,String serviceCode,
		...
		String dossierTemplateNo,String password,int viaPostal,String postalAddress,String postalCityCode,
		String postalCityName,String postalDistrictCode,String postalDistrictName,String postalTelNo,boolean online,
		boolean notification,String applicantNote,int originality,Date createDate,Date modifiedDate,Date submitDate,
		Date receiveDate,Date dueDate,Date releaseDate,Date finishDate,Date cancellingDate,Date correctingDate,
		Date endorsementDate,Date extendDate,Date processDate,String dossierNo,String dossierStatus,
		String dossierStatusText,String dossierSubStatus,String dossierSubStatusText,long dossierActionId,
		String submissionNote,String lockState,String delegateName,String delegateIdNo,String delegateTelNo,
		String delegateEmail,String delegateAddress,String delegateCityCode,String delegateCityName,
		String delegateDistrictCode,String delegateDistrictName,String delegateWardCode,String delegateWardName,
		double durationCount,int durationUnit,String dossierName,String processNo,String metaData,Integer vnpostalStauts,
		String vnpostalProfile,Integer fromViaPostal,ServiceContext context) throws PortalException
	{
		...
		try
		{
			//Process
			dossier = DossierLocalServiceUtil
				.publishDossier(groupId,dossierId,referenceUid,counter,serviceCode,serviceName,govAgencyCode,govAgencyName,
					applicantName,applicantIdType,applicantIdNo,appIdDate,address,cityCode,cityName,districtCode,districtName,
					wardCode,wardName,contactName,contactTelNo,contactEmail,dossierTemplateNo,password,viaPostal,postalAddress,
					postalCityCode,postalCityName,postalDistrictCode,postalDistrictName,postalTelNo,online,notification,
					applicantNote,originality,createDate,modifiedDate,submitDate,receiveDate,dueDate,releaseDate,finishDate,
					cancellingDate,correctingDate,endorsementDate,extendDate,processDate,dossierNo,dossierStatus,
					dossierStatusText,dossierSubStatus,dossierSubStatusText,dossierActionId,submissionNote,lockState,delegateName,
					delegateIdNo,delegateTelNo,delegateEmail,delegateAddress,delegateCityCode,delegateCityName,
					delegateDistrictCode,delegateDistrictName,delegateWardCode,delegateWardName,durationCount,durationUnit,
					dossierName,processNo,metaData,vnpostalStauts,vnpostalProfile,fromViaPostal,context);
	...
	@Override
	public Dossier publishImportDossier(long groupId,long dossierId,String referenceUid,int counter,String serviceCode,
		String serviceName,String govAgencyCode,String govAgencyName,String applicantName,String applicantType,
		String applicantIdNo,Date applicantIdDate,String address,String contactName,String contactTelNo,String contactEmail,
		String dossierNo,String dossierStatus,String dossierStatusText,Boolean online,int originality,int sampleCount,
		Double durationCount,Integer durationUnit,Date createDate,Date modifiedDate,Date submitDate,Date receiveDate,
		Date dueDate,Date releaseDate,Date finishDate,String dossierTemplateNo,String dossierTemplateName,
		Integer vnpostalStatus,String vnpostalProfile,Integer fromViaPostal,ServiceContext serviceContext)
	{
		...
		try
		{
			...
			if (dossierId == 0)
			{
				...
				dossierAction = DossierActionLocalServiceUtil
					.updateImportDossierAction(groupId,0l,option != null ? option.getServiceProcessId() : 0,fromStepCode,
						fromStepName,fromSequenceNo,actionCode,actionUser,actionName,stepCode,stepName,null,0l,1,serviceContext);
			}
			else
			{
			...
			//
			dossier = DossierLocalServiceUtil
				.publishImportDossier(groupId,dossierId,referenceUid,counter,serviceCode,serviceName,govAgencyCode,
					govAgencyName,applicantName,applicantType,applicantIdNo,applicantIdDate,address,contactName,contactTelNo,
					contactEmail,online,originality,dossierNo,dossierStatus,dossierStatusText,
					dossierAction != null ? dossierAction.getDossierActionId() : 0,durationCount,durationUnit,sampleCount,
					createDate,modifiedDate,submitDate,receiveDate,dueDate,releaseDate,finishDate,dossierTemplateNo,
					dossierTemplateName,vnpostalStatus,vnpostalProfile,fromViaPostal,serviceContext);
	...
	@Override public Dossier updateDossierVNPost(long groupId ,String ma_bien_nhan,int viaPostal,String postalServiceCode,
		...
		if (Validator.isNotNull(ma_bien_nhan) && receiptCodeSplit.length == 4 && checkKey.equals("D"))
		{
			...
			if (Validator.isNotNull(securityCode) && Validator.isNotNull(dossierId) && Validator.isNotNull(dossierCounter))
			{
				...
				if (Validator.isNotNull(dossiers) && dossiers.size() == 1)
				{
					...
					if (Validator.isNotNull(postalServiceCode))
					{
						dossier.setPostalServiceCode(postalServiceCode);
					}
					if (Validator.isNotNull(postalServiceName))
					{
					...
					if (Validator.isNotNull(postalWardCode))
					{
						dossier.setPostalWardCode(postalWardCode);
					}
					if (Validator.isNotNull(postalWardName))
					{
