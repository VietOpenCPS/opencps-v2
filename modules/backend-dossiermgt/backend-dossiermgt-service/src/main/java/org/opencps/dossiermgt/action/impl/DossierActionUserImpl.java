...
//		_log.info("END ROLES");
	...
	private void updateDossierUser(Dossier dossier, ProcessStepRole processStepRole, User user) {
		...
		if (du == null) {
			DossierUserLocalServiceUtil.addDossierUser(dossier.getGroupId(), dossier.getDossierId(), user.getUserId(),
			...
			try {
				if ((processStepRole.getModerator() && du.getModerator() != DossierActionUserTerm.ASSIGNED_PH)
					...
					DossierUserLocalServiceUtil.updateDossierUser(dossier.getDossierId(), user.getUserId(),
	...
	private void initDossierActionUserByServiceProcessRole(Dossier dossier, int allowAssignUser,
		...
		try {
			ServiceProcess serviceProcess = ServiceProcessLocalServiceUtil.getServiceByCode(groupId,
					dossier.getServiceCode(), dossier.getGovAgencyCode(), dossier.getDossierTemplateNo());
			List<ServiceProcessRole> listSprs = ServiceProcessRoleLocalServiceUtil
			...
			for (ServiceProcessRole spr : listSprs) {
				...
				for (User user : users) {
					...
					if (dau != null) {
						...
						addDossierActionUserByAssigned(allowAssignUser, user.getUserId(),
								dossierAction.getDossierActionId(), mod, false, da.getStepCode(),
								dossier.getDossierId(), 0, 0);
					}
				}
			}
...
//		DossierActionUserLocalServiceUtil.addDossierActionUser(model);
		...
		List<org.opencps.dossiermgt.model.DossierActionUser> lstDaus = DossierActionUserLocalServiceUtil
			...
			if (subUser != null && subUser.has(DossierActionUserTerm.ASSIGNED)
				...
				if (dossierUser != null) {
					...
					if (allowAssignUser != ProcessActionTerm.NOT_ASSIGNED) {
						...
						DossierUserLocalServiceUtil.updateDossierUser(dossierUser.getDossierId(),
								dossierUser.getUserId(), dossierUser.getModerator(), dossierUser.getVisited());
					...
					if (allowAssignUser != ProcessActionTerm.NOT_ASSIGNED) {
						DossierUserLocalServiceUtil.addDossierUser(groupId, dossier.getDossierId(), userIdAssigned, 1,
								true);
					}
				}
...
//					DossierAction dAction = DossierActionLocalServiceUtil.fetchDossierAction(dossierAction.getDossierActionId());
					...
					if (dAction != null) {
						addDossierActionUserByAssigned(allowAssignUser, userIdAssigned,
								dossierAction.getDossierActionId(), moderator, false, dAction.getStepCode(),
								dossier.getDossierId(), delegacy, assigned);
					}
//					else {
