...
public class ServiceInfoLocalServiceImpl extends ServiceInfoLocalServiceBaseImpl {
	...
	public Hits searchLucene(LinkedHashMap<String, Object> params, Sort[] sorts, int start, int end,
		...
		if (Validator.isNotNull(keywords)) {
			...
			String[] subQuerieArr = new String[] { ServiceInfoTerm.SERVICE_NAME_SEARCH, ServiceInfoTerm.SERVICE_NAME,
					ServiceInfoTerm.SERVICE_CODE_SEARCH, ServiceInfoTerm.DOMAIN_CODE_SEARCH,
					ServiceInfoTerm.ADMINISTRATION_CODE_SEARCH };
	...
	public ServiceInfo adminProcessData(JSONObject objectData) {
		...
		if (objectData.getLong(ServiceInfoTerm.SERVICE_INFO_ID) > 0) {
			...
			try {
				valdiate(objectData.getString(ServiceInfoTerm.SERVICE_CODE), objectData.getString("serviceName"),
						objectData.getString(ServiceInfoTerm.ADMINISTRATION_CODE), objectData.getString("domainCode"),
		...
		if (objectData.has(ServiceInfoTerm.PUBLIC_)) {
			_log.debug("SERVICE INFO ADMIN DATA PUBLIC: " + objectData.getBoolean(ServiceInfoTerm.PUBLIC_));
		}
		if (objectData.has(ServiceInfoTerm.IS_NOTARIZATION)) {
			...
			_log.debug("SERVICE INFO ADMIN DATA IS NOTARIZATION: " + objectData.getBoolean(ServiceInfoTerm.IS_NOTARIZATION));
		}
