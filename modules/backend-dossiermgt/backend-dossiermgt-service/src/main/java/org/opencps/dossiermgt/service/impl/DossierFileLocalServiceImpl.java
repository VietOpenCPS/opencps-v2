...
public class DossierFileLocalServiceImpl extends DossierFileLocalServiceBaseImpl {
	 ...
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never reference this class directly. Always use {@link org.opencps.dossiermgt.service.DossierFileLocalServiceUtil} to access the dossier file local service.
	 */
	...
	public DossierFile cloneDossierFile(long groupId, long dossierId, 
		...
		if(dossierFile.getFileEntryId() > 0) {
			try {
				FileEntry fileEntry = FileUploadUtils.cloneDossierFile(user.getPrimaryKey(), 
					groupId, dossierFile.getFileEntryId(), serviceContext);
	...
	public void cloneDossierFilesByDossierId(long groupId, long newDossierId, 
		...
		List<DossierFile> dossierFiles = dossierFileLocalService.getDossierFilesByD_DP(
			oldDossierId, dossierPartType);

		for(DossierFile dossierFile : dossierFiles) {
			cloneDossierFile(groupId, newDossierId, dossierFile.getDossierFileId(), 
	...
	public DossierFile updateFormData(long groupId, long dossierId, String referenceUid, String formData,
		...
		if(Validator.isNull(jrxmlTemplate)) {
			DossierPart dossierPart = dossierPartLocalService.fetchByTemplatePartNo(
				groupId, dossierFile.getDossierTemplateNo(), dossierFile.getDossierPartNo());
