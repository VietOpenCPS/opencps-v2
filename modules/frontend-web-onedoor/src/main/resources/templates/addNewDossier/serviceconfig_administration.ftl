...
<script type = "text/javascript">
	...
	var fnGetParamAndCreateDossier =  function(processOptionId, templateNo){
		...
		if(templateNo && serviceConfigId){
			...
			$.ajax({
				...
				success : function(result){
					if(result){
						fnCreateDossier(dossierTemplateNo, result.serviceCode, result.govAgencyCode, result.servicerName);
					}
				},
				error : function(result){
	...
	var fnCreateDossier = function(dossierTemplateNo,serviceCode,govAgencyCode,servicerName){
		dossierObject.dossierTemplateNo = dossierTemplateNo;
		dossierObject.serviceCode = serviceCode;
		dossierObject.servicerName = servicerName;
		dossierObject.govAgencyCode = govAgencyCode;
		addDossierRouter.navigate("/buoc2");
	};
	...
	$("#listView").kendoListView({
		...
		dataBound : function(){
			...
			$(this.element).find(".dropdown-menu").each(function(){
				var id = $(this).attr("data-pk");
