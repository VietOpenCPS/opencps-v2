	...
	<ul id="employee_menu">
		<li class="k-state-active">
			...
			<div class="row">
				...
				<div class="col-sm-12">
					<div class="form-group">
						<input class="form-control" name="ownerProfile" id="ownerProfile" placeholder="Tên chủ hồ sơ">
					</div>
				</div>
				<div class="col-sm-12">
					<div class="form-group">
...
<script type="text/javascript">
	...
	$("#employee_menu").kendoPanelBar({ animation: {} });
	...
	$(function(){
		var arrStatus=${employee.status};
		$("#profileStatus").empty();
		for(var i=0;i<arrStatus.length;i++){
			$("#profileStatus").append("<li style='padding:5px 0;' dataPk='"+arrStatus[i].statusCode+"' class='itemStatus'><a href='javascript:;' >"+arrStatus[i].statusName+"  ("+arrStatus[i].statusCount+")</a></li>");
		}
		$("#profileStatus li").first().addClass("active");

		$("#employeemain_dossierlist").load("${employee.ajax.employeemain_dossierlist}",function(result){
			var id=$("#profileStatus li").first().attr("dataPk");
			dataSourceProfile.read({
				"serviceInfo":$("#serviceInfo").val(),
				"ownerProfile":$("#ownerProfile").val(),
	...
	$(document).on("click",".itemStatus",function(event){
		...
		$("#employeemain_dossierlist").load("${employee.ajax.employeemain_dossierlist}",function(result){
			$("#listViewProfile").getKendoListView().dataSource.read({
				"serviceInfo":$("#serviceInfo").val(),
				"ownerProfile":$("#ownerProfile").val(),
				"receiptCode":$("#receiptCode").val(),
				"startDate":$("#startDate").val(),
